...
cal_weights <- function(M,svd.M,keepX=NA,keepY=NA,groupX=NA,groupY=NA,subgroupX=NA,subgroupY=NA,
                                          ...
                                          lambda.x=0, lambda.y=0,structured = FALSE,group_obs,n_study){
  ...
  # lamb.upper.x = 2*sum(abs(Mv))/abs(1-alpha1.x - alpha2.x) + 1
  # lamb.upper.x = min(lamb.upper.x)
  lamb.upper.x = 2*max(abs(Mv))/abs(1-alpha1.x - alpha2.x) + 1
  lamb.upper.x = min(lamb.upper.x,10^15)
  ...
  if (structured){
    dim_var1 <- dim(M)[1]/n_study
    dim_var2 <- dim(M)[2]/n_study
  }
  repeat{
    ittr = ittr + 1
    ...
    # if(structured){
    #   max_dim<- max(unlist(lapply(1:n_study,function(i){length(which(group_obs == i))})))
    #     dim_var <- length(Mv)/n_study
    # 
    #   for(iobs in 1:n_study){
    #     Mv[ 1:dim_var  + (iobs-1)*dim_var] <- Mv[ 1:dim_var  + (iobs-1)*dim_var] / sqrt(length(which(group_obs == iobs))) *sqrt(max_dim)
    #   }
    # 
    # }
    ...
    if (structured){
      ...
      for (is in 1:n_study){
        u[ 1:dim_var  + (is-1)*dim_var] <- scale_vec(u[ 1:dim_var  + (is-1)*dim_var] , scale =0.5)
      }
     
    } else {u = scale_vec(u, scale = 0.5)}
    ...
    if (structured){
      dim_var <- length(Mu)/n_study
      for (is in 1:n_study){
        v[ 1:dim_var  + (is-1)*dim_var] <- scale_vec(v[ 1:dim_var  + (is-1)*dim_var] , scale = 0.5)
      }
      
    } else {v = scale_vec(v, scale = 0.5)}
    # Check convergence
