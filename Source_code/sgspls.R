...
shift <- function(X,s,group_obs){
  ...
  # group_obs : vector of size dim(X)[1] giving the number of the study of the observations
  
  temp_p <- dim(X)[2]
  X_shift <- matrix(0,ncol = temp_p*s,nrow = dim(X)[1])
  for (is in 1:s){
    X_shift[which(group_obs == is), 1:temp_p + (is-1)*temp_p] <- X[which(group_obs == is),] 
  }
  return(X_shift)                        
}

unshift <- function(X,s,group_obs){
  ...
  temp_p <- dim(X)[2]/s
  X_unshift <- matrix(0,ncol = temp_p,nrow = dim(X)[1])
  for (is in 1:s){
    X_unshift[which(group_obs == is),]  <- X[which(group_obs == is), 1:temp_p + (is-1)*temp_p] 
  }
  return(X_unshift)                        
}
...
sgspls <-
  function(X, Y, ncomp = 2, mode = "regression",  
           ...
           groupX = rep(1,ncol(X)), groupY = rep(1,ncol(Y)), group_obs=rep(1,nrow(X)),
           ...
           da = FALSE, structured = FALSE, n_study =1,
           ...)
  {
    ...
    # if (any(!is.finite(as.matrix(Y)))) stop("\ninfinite, NA or NaN values in X or Y")
    if (any(!is.finite(unlist(Y)))) stop("\ninfinite, NA or NaN values in X or Y")
    ...
    X_h <- X
    if (n_study > 1){
      max_group_obs <- n_study
      for (iobs in 1:max_group_obs){
        Y_h[which(group_obs == iobs),] <- scale(Y[which(group_obs == iobs),], center = T, scale = scale.y)
        X_h[which(group_obs == iobs),] <- scale(X[which(group_obs == iobs),], center = T, scale = scale.x)
      }
      Y_h[is.na(Y_h)]<-0
      X_h[is.na(X_h)]<-0
    } else {
    ...
    for(h in 1:ncomp){
      ...
      if(structured){
        u_structured <- NULL
        v_structured <- NULL
        for (istudy in 1:n_study){
          svd_temp <- svd(M[1:(p/n_study)+(istudy-1)*p/n_study,1:(q/n_study)+(istudy-1)*q/n_study],nu = 1,nv = 1)
          u_structured <- c(u_structured,svd_temp$u)
          v_structured <- c(v_structured,svd_temp$v)
        }
        svd.M <- list(u = u_structured,
                      v = v_structured)
      }
      ...
      weights <- cal_weights(M = M,svd.M = svd.M,keepX = keepX[h],keepY = keepY[h],groupX = groupX,
                                                groupY = groupY,subgroupX = subgroupX,subgroupY = subgroupY, group_obs= group_obs, alpha1.x = alpha1.x[h],
                                                alpha2.x = alpha2.x[h],alpha1.y = alpha1.y[h],alpha2.y = alpha2.y[h],tol, max.iter,
                                                lambda.x = lambda.x[h], lambda.y = lambda.y[h], structured = structured,n_study = n_study)
      ...
      if(structured){
        ...
        max_group_obs <- n_study
        for(iobs in 1:max_group_obs){
          X_h[which(group_obs == iobs), -( 1:dim(X_h)[2]/max_group_obs + (iobs-1)*dim(X_h)[2]/max_group_obs) ] <- 0
          Y_h[which(group_obs == iobs), -( 1:dim(X_h)[2]/max_group_obs + (iobs-1)*dim(X_h)[2]/max_group_obs) ] <- 0
        }
      }
    }
    cl = match.call()
    ...
    parameters <- list(X=X,
                       ...
                       keepX=keepX,
                       keepY=keepY,
                       groupX = groupX, groupY = groupY, group_obs = group_obs,
                       ...
                       da = da, structured =structured, n_study= n_study)
