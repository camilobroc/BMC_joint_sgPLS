  ...
  function(pls_obj, sparsities=NULL, group_seq=NULL, block = "X",
    ...
    for( j in 1:nsparsities){
      ...
      object_groups <- tune_groups(parameters, group_seq=group_seq, setseed = setseed, 
                                   block = block, scale_resp = scale_resp, folds = folds)
      results <- cbind( group_seq, sparsities[j,3], sparsities[j,2], t(object_groups$MSEP[ncomp, ,drop=F]), t(object_groups$MSEP_var[ncomp, ,drop=F])  )
    ...
    colnames(results_tuning) <- c("Groups", "Individual", "Subgroup", paste0("MSEP comp",ncomp),paste0("MSEP var comp",ncomp))
    
    MSEPopt <- min(results_tuning[,4])
    temp_min <- which(results_tuning[which(results_tuning[,4] == MSEPopt, arr.ind = T),"Groups"] == min(results_tuning[which(results_tuning[,4] == MSEPopt, arr.ind = T),"Groups"]))
     best <- results_tuning[which(results_tuning[,4] == MSEPopt, arr.ind = T)[temp_min[1]],]
    keep <- best[1];
...
tune_groups <-
  function(parameters, group_seq = NULL, block = "X", folds = 10, scale_resp = TRUE, setseed = 1) {
    ...
    MSEP_var <- matrix(0, ncol = ngroups, nrow = max(ncomp))
    ...
    for(i in 1:ngroups){
      ...
      MSEP_var[ , i ] <-object_perf$MSEP_var
      gc()
    }
    ...
    if(block == "X" || block == "x")
      object$parameters$keepX[ncomp] <- keep_opt
    
    if(block == "Y" || block == "y")
      object$parameters$keepY[ncomp] <- keep_opt

    res <- list(
      MSEP = MSEP, MSEP_var = MSEP_var, ncomp_opt = ncomp_opt, keep_opt = keep_opt, parameters = object$parameters, folds = object_perf$folds
    )
    return(res)
  }
