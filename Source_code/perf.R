...
perf.sgspls <- 
  ...
  pls_parameters$X.residuals <-  NULL
  pls_parameters$Y.residuals <-  NULL
  X <- pls_parameters$X
  Y <- pls_parameters$Y
          ...
          if (is.null(folds) || !is.numeric(folds) || folds < 2 || folds > n)
            {
              ...
              if(object$parameters$structured){
                # max_group_obs <- max(object$parameters$n_study)
                M = round(folds)
                
                
                
                # temp_sample0 <- split(1:n,object$parameters$group_obs)
                # temp_sample1 <- lapply(temp_sample0,function(x){lapply(split(x,Y[x,1]),sample)})
                # temp_sample <-unlist(temp_sample1)

                if(object$parameters$da){ 
                  split(1:n,object$parameters$group_obs) %>% lapply(function(x){split(x,object$parameters$Y[x,1])}) %>% lapply(function(x){lapply(x,sample)})
                  temp_sample <-  split(1:n,object$parameters$group_obs) %>%
                    lapply(function(x){split(x,object$parameters$Y[x,1])}) %>% lapply(function(x){lapply(x,sample)}) %>% 
                    unlist() %>% unlist()
                  names(temp_sample)<-NULL
                } else {
                  temp_sample <- split(1:n,object$parameters$group_obs) %>% 
                    lapply(sample) %>% unlist()
                  names(temp_sample)<-NULL
                }
                
              } else {
                M = round(folds)
                # temp_sample <- sample(1:n)
                
                # temp_sample <- split(1:n,object$parameters$Y[,1])
                # temp_sample0 <- 1:n
                # temp_sample1 <-  lapply(temp_sample0,sample)
                # temp_sample <-  unlist(temp_sample1)
                # names(temp_sample)<-NULL
              ...
              folds = temp_sample %>% split(rep(1:M,length=n))
            }
        }
    } else {
      
      folds = lapply(1:n,function(x){x})
      M = n
    }
  ...
  for (i in 1:M) {
    ...
    pls_parameters$group_obs = object$parameters$group_obs[-omit]
    pls_parameters$n_study = object$parameters$n_study
    group_obs_test <- object$parameters$group_obs[omit]
    #-- fit the PLS method using the cv dataset ---#
    
    spls_res = do.call(sgspls, args = pls_parameters)
    
    Y_hat =  predict(spls_res, X_test, group_obs_test = group_obs_test)$pred
  ...
  nonzX <- NULL
  Y_hat <- predict(object, X,group_obs_test = group_obs_test)
  
  MSEP_var<-  rep(-1,ncomp)
  for (h in 1:ncomp) 
    {
    ...
    temp_MSEP_var1 <- matrix(ncol = q,nrow =length(folds))
    
    for( i in 1:M){
      temp_MSEP_var1[i,]<- apply(as.matrix(RSS_indiv[folds[[i]], , h]), 2, mean, na.rm = TRUE)
    }
    temp_MSEP_var2 <-  apply(temp_MSEP_var1, 1, mean)
    
    temp_MSEP_var3 <- sd(temp_MSEP_var2)
    
    MSEP_var[ h] = temp_MSEP_var3 
  ...
  if (progressBar == TRUE)
    cat("\n")
  res <- list(
    MSEP = MSEP, MSEP_var=MSEP_var, PRESS0 = PRESS0, PRESS = PRESS, R2 = R2,
